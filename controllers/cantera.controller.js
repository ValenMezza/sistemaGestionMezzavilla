...
const { streamResumenPDF, money } = require('../utils/pdf');
...
exports.viewMensual = async (req, res) => {
  ...
  let sales = await store.listSalesByTypeBetween('CANTERA', from, to);
  ...
  const salesList = await Promise.all(sales.map(async (s) => {
...
exports.viewResumenDia = async (req, res) => {
  ...
  const clients = await store.listClients();
  const products = (await store.listProducts()).filter((p) => p.active);
  let salesRaw = await store.listSalesByTypeOnDay('CANTERA', dayISO, clientId || null);
  ...
  const filteredSales = (conDeuda === 'on' || conDeuda === 'true')
    ? sales.filter(s => (toNum(s.final_price) - toNum(s.paid_amount)) > 0)
    : sales;

  const summary = {
    ventas: filteredSales.length,
    efectivo: filteredSales.filter(x => x.payment_method === 'efectivo').reduce((a, s) => a + toNum(s.final_price), 0),
    transferencia: filteredSales.filter(x => x.payment_method === 'transferencia').reduce((a, s) => a + toNum(s.final_price), 0),
    a_cobrar: filteredSales
...
exports.pdfSemanal = async (req, res) => {
  ...
  if (!weekStart) return res.redirect('/cantera/semanal');

  // calcular weekEnd con util existente
  ...
  return streamResumenPDF(res, { title: 'Cantera - Resumen semanal', subtitle, kpis, rows });
};
...
exports.pdfResumenDia = async (req, res) => {
  ...
  return streamResumenPDF(res, { title: 'Cantera - Resumen del día', subtitle, kpis, rows });
};
...
exports.pdfMensual = async (req, res) => {
  ...
  return streamResumenPDF(res, { title: 'Cantera - Resumen mensual', subtitle, kpis, rows });
};
